...
const solHigh = { ...sol, thinkingLevel: "high" as const };
const lunaLow = { ...luna, thinkingLevel: "low" as const };

test("an enabled new session inherits the active model once", () => {
	const handoff = new NewSessionModelHandoff();
	handoff.beforeSwitch("new", sol, "high");
	assert.deepEqual(handoff.consume("new", true), solHigh);
...
test("a later switch replaces stale or cancelled new-session state", () => {
	const handoff = new NewSessionModelHandoff();
	handoff.beforeSwitch("new", sol, "high");
	handoff.beforeSwitch("resume", luna, "medium");
	assert.equal(handoff.consume("resume", true), undefined);

	handoff.beforeSwitch("new", sol, "high");
	handoff.beforeSwitch("new", luna, "low");
	assert.deepEqual(handoff.consume("new", true), lunaLow);
});
